...
import com.example.hotelmanagement.db.HelpFunctions;
            ...
            if (!orderCBox.getValue().equals("New Order")){
                try {
                    ...
                    while(rs.next()){
                        ...
                        fromDatePicker.setValue(rs.getDate("fromDate").toLocalDate());

                    }

                    rs = DBInterface.getResultSet("SELECT toDate FROM order_room where id = " + orderCBox.getValue());
                    while(rs.next()){
                         toDatePicker.setValue(rs.getDate("toDate").toLocalDate());
                    }
    ...
    */
    ...
    void handleAddApplyOrderAction(ActionEvent event) throws IOException {
        if (guestCBox.getSelectionModel().isEmpty() || roomCBox.getSelectionModel().isEmpty() || !dateError.getText().equals("")){
            ...
            c1.setTime(Date.valueOf(fromDatePicker.getValue()));

            Calendar c2 = Calendar.getInstance();
            c2.setTime(Date.valueOf(toDatePicker.getValue()));
            int nextId;
            if (orderCBox.getValue().equals("New Order")){
    ...
    public void createBill(MouseEvent arg0) throws IOException {
       if (!orderCBox.getValue().equals("New Order")){
           ...
           try {
               ...
               html += "<td>" + fromDatePicker.getValue() + "</td>";
               html += "<td>" + toDatePicker.getValue() + "</td>";
...
//        toDatePicker.alignmentProperty().set(Pos.CENTER);
        ...
        toDatePicker.valueProperty().addListener(new InvalidationListener() {
            @Override
        ...
        StringConverter<LocalDate> fromConverter = new StringConverter<LocalDate>() {
            DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
       ...
       // fromDatePicker.alignmentProperty().set(Pos.CENTER);
        ...
        fromDatePicker.valueProperty().addListener(new InvalidationListener() {
            @Override
            public void invalidated(Observable o) {
               checkDates();
            }
